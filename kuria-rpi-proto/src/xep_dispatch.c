...
uint32_t dispatch_register(void** queue, XepDispatch_t* dispatch, uint32_t queue_length)
{
...
uint32_t dispatch_subscribe(XepDispatch_t* dispatch, XepDispatchMessageTags_t message_tag, void* queue)
{
...
uint32_t dispatch_reply_message(uint32_t* message_id, XepDispatch_t* dispatch, XepDispatchMessage_t* received_message, XepDispatchMessageTags_t message_tag, MemoryBlock_t *memoryblock, uint32_t length)
{
	...
	QueueHandle_t queue = NULL; //dispatch->subscribers[message_tag].queue;
