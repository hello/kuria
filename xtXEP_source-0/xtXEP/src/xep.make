...
C_HAL_SRCS += \
    ...
    hal/$(HW_PLATFORM)/sdramc.c \
	hal/$(HW_PLATFORM)/qspi.c \
    hal/$(HW_PLATFORM)/usbhs_device.c \
...
FILES_TO_CLEAN = $(C_OBJECTS) $(C_XTESRIAL_OBJECTS) $(C_ASF_OBJECTS) \
...
$(TARGET_FOLDER)/$(PRODUCT_NAME)_$(HW_PLATFORM).elf: $(C_OBJECTS) $(C_HAL_OBJECTS) $(C_XTSERIAL_OBJECTS) $(C_ASF_OBJECTS) $(C_FREERTOS_OBJECTS) $(CPP_OBJECTS) $(PARAMS_IN_SOURCE_OBJECTS)
	...
	$(CXX) $^ -march=armv7e-m -mthumb -Wl,-Map=$(TARGET_FOLDER)/$(PRODUCT_NAME)_$(HW_PLATFORM).map $(LD_FLAGS) -Wl,--start-group -larm_cortexM7lfsp_math -lm -Wl,--end-group -Wl,--gc-sections -Wl,--entry=Reset_Handler -Wl,--cref -Wl,--undefined=uxTopUsedPriority -mthumb -L$(TOP_LEVEL_DIR)/src/hal/$(HW_PLATFORM) -Tapplication_without_bl.ld -o $@
