    ...
    const unsigned char byte)
{
    if (instance->packetMode == MCP_PACKETMODE_NOESCAPE)
    {
        ...
        const uint32_t packet_length = *(uint32_t*)(&instance->destination[0]);
    ...
    void * user_data)
{
    ...
    process_byte(0xf0, callback, user_data);
    process_byte(0x5e, callback, user_data);
    process_byte(0xb9, callback, user_data);
    process_byte(0xa2, callback, user_data);
    packet_end(callback, user_data);
    return 0;
}
    ...
    void * user_data)
{
    ...
    process_bytes((unsigned char *)filename, filename_length, callback, user_data);
    packet_end(callback, user_data);
    return 0;
}
    ...
    void * user_data)
{
    ...
    process_int(fps, callback, user_data);
    packet_end(callback, user_data);
    return 0;
}
